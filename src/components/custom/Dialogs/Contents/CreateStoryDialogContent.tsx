...
function CreatePostDialogContent() {
  ...
  function onSubmit() {
   ...
   if(files.length!==0)
   {
    ...
    clientapi.post('/story/create',formData,{headers:{"Content-Type":'multipart/form-data'}}).then((res)=>{
      if(res.status=201)
      {
        form.reset();
        setFiles([]);
        toast({title:"story created successfully ",description:res.data.message})
      }
